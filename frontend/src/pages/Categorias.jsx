...
import ConfirmDialog from '../components/ConfirmDialog'; // Importamos tu componente de confirmación
...
function Categorias() {
  ...
  // Estado para confirmación de desactivación
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
            ...
            <tbody className="divide-y divide-slate-50">
              {currentItems.map((cat) => (
                <tr key={cat.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-8 py-5">
                    <p className="text-slate-500 text-xs font-medium max-w-xs truncate" title={cat.descripcion}>
          ...
          <div className="bg-white rounded-[2.5rem] p-6 md:p-10 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1">Nombre *</label>
                <input
                  required
                  type="text"
                  value={formData.nombre}
                  onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-4 focus:ring-[#003366]/5 transition-all text-sm font-medium"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-4 focus:ring-[#003366]/5 transition-all h-28 resize-none text-sm font-medium"
                />
              </div>

              <button type="submit" className="w-full py-3.5 bg-[#003366] text-white font-bold rounded-xl hover:bg-[#001a33] shadow-lg transition-all active:scale-[0.98]">
      ...
      <ConfirmDialog 
        ...
        message="La categoría dejará de estar disponible para nuevos productos, pero se mantendrá en los registros existentes."

...
function Marcas() {
  ...
  // OPTIMIZACIÓN: Eliminación de retardo artificial para carga instantánea
  const loadMarcas = useCallback(async (isFirstLoad = false) => {
    if (isFirstLoad) setLoading(true);
    try {
      const response = await marcasAPI.getAll();
      setMarcas(response.data);
    } catch (error) {
      setNotification({ message: 'Error al conectar con las marcas', type: 'error' });
    } finally {
      setLoading(false); // CARGA INSTANTÁNEA: Sin setTimeout
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      loadMarcas(); // Recarga rápida en segundo plano
      closeModal();
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
            ...
            <tbody className="divide-y divide-slate-100">
              {currentItems.length > 0 ? (
                currentItems.map((marca) => (
                  <tr key={marca.id} className="hover:bg-slate-50/50 transition-colors flex flex-col md:table-row p-6 md:p-0">
                    <td className="px-0 md:px-8 py-2 md:py-5">
                      <div className="flex items-center justify-between md:justify-start gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="font-bold text-slate-800 uppercase text-sm tracking-tight whitespace-nowrap">{marca.nombre}</div>
                        </div>
                      </div>
                    </td>

                    <td className="px-0 md:px-8 py-2 md:py-5 border-t border-slate-50 md:border-none mt-1 md:mt-0 pt-3 md:pt-5">
                      <div className="flex justify-between md:block gap-4">
                        ...
                        <p className="text-slate-500 text-xs font-medium max-w-[180px] md:max-w-xs truncate text-right md:text-left" title={marca.descripcion}>
          ...
          <div className="bg-white rounded-[2.5rem] p-6 md:p-10 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input required type="text" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} placeholder="Ej: LG, York..." className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-4 focus:ring-[#003366]/5 transition-all text-sm font-medium" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex justify-center pt-3">
                ...
                >
                  {editingMarca ? 'Guardar Cambios' : 'Registrar'}
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmDialog
        ...
        message="La marca no aparecerá en nuevos productos, pero se mantendrá el historial."
